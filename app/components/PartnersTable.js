...
export default function PartnersTable({ partners }) {
  if (!partners.length) {
    return <p className="text-sm text-slate-500">No hay socios registrados.</p>;
  }
          ...
          {partners.map((p) => {
            ...
            return (
              <tr key={p.id} className="hover:bg-slate-50">
                ...
                <td className="px-4 py-3">{fmt(p.capitalContributed)}</td>
                <td className="px-4 py-3">{p.participationPct.toFixed(2)}%</td>
                <td
                ...
                <td className="px-4 py-3 font-medium">{fmt(p.currentValue)}</td>
              </tr>
            );
          })}
        </tbody>
