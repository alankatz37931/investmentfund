            ...
            {positions.map((p) =>
              editingId === p.id ? (
                ...
                <tr key={p.id} className="hover:bg-slate-50">
                  ...
                  <td className="px-4 py-3">{Number(p.quantity)}</td>
                  <td className="px-4 py-3">{fmt(p.avg_buy_price)}</td>
      ...
      >
        ...
        <div className="mt-3 flex flex-wrap items-end gap-3">
          <input
            ...
            className="w-32 rounded border border-slate-300 px-3 py-2 text-sm font-semibold uppercase"
          />
          <input
            type="number"
            required
