...
export async function getQuotes(tickers) {
  ...
  for (const ticker of tickers) {
    ...
    // Prioridad: dailyBar.c (close del bar diario, estable y representativo)
    //         → prevDailyBar.c (si todavía no hay bar de hoy, pre-market temprano)
    //         → latestTrade.p (último recurso, puede ser un print sparse de IEX after-hours)
    // En IEX free el dailyBar es más confiable que latestTrade — evita ver prints viejos
    // de after-hours cuando el mercado oficial cerró en otro precio.
    const latestPrice =
