...
export async function getQuotes(tickers) {
  ...
  for (const ticker of tickers) {
    ...
    // Estrategia híbrida por freshness del último trade:
    //  - Si latestTrade es reciente (< 4h) → usarlo (captura after-hours en IEX cuando los hay)
    //  - Si no → dailyBar.c (close estable, evita prints stale de hace 12+ horas)
    //  - Fallbacks: prevDailyBar.c y por último latestTrade.p
    const FRESH_TRADE_MS = 4 * 60 * 60 * 1000;
    const tradeTimeMs = snap.latestTrade?.t
      ? new Date(snap.latestTrade.t).getTime()
      : 0;
    const tradeIsFresh =
      snap.latestTrade?.p != null &&
      tradeTimeMs > 0 &&
      Date.now() - tradeTimeMs < FRESH_TRADE_MS;

    const latestPrice = tradeIsFresh
      ? snap.latestTrade.p
      : snap.dailyBar?.c ?? snap.prevDailyBar?.c ?? snap.latestTrade?.p ?? 0;
